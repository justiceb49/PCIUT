...
struct PCIUT3100 {
	int		sampleRate,	//0=100MHz, 1=50MHz...
			triggerSrc,	//0=+Ext, 1=Software trigger, 2=-Ext
			lpf,		//0=No Filter, 1=48 MHz, 2=28 MHz, 
						//3=18 Mhz, 4=8.8 MHz, 5=7.5 MHz, 
						//6=6.7 MHz, 7=5.9 MHz
			hpf,		//0=4.8 MHz, 1=1.8 MHz, 2=0.8 MHz, 3=0.6 MHz
			signal,		//0=Full 1=+half; 2=-half; 3=RF
			damping,	//for single channel board only: 
						//0=620 ohms,1=339,2=202,3=159,4=60,5=55,6=50,7=47
			pe,			//for single channel board only: 
						//0=single transducer, 1=dual transducers
			plChannel,	//0 to 255
			rxChannel,	//0 to 255
			outv,		//0 to 255
			width,		//0 to 255
			gain,		//0 - 1023
			triggerFreq,//0 to 5000 Hz set internal trigger mode if > 0
			dcoffset,	//0 - 1023
			len,		// buffer length 16 to 16382 in step of 4 	
			post,		// trigger delay 2 to 32764 in step of 2
			logamp,		// logarithmic amplifier: 1=On, or 0=Off
			TBCycles,	//Tone Burst Cycles
			reserved[6];
};
...
#define SETPRF				1038
	...
	// lParam: not used
	// Note: This command is for PCIPR300 card only, not for PCIUT3100.
...
#define GETINFO	1047
	// wParam: pointer of char*
	// lParam: 1: get sampling rate, 2-9: LPF labels, 10-13: HPF labels

#define SETTONEBURSTCYCLES		1900
	// wParam: cycles of 2nd burst, 1 to 511, 0=no 2nd and no 3rd bursts
	// lParam: cycles of 3rd burst, 1 to 511, 0=no 3rd bursts

#define SETTONEBURSTGAPS		1901
	// wParam: time of gap t4 in clock cycles
	// lParam: time fo gap t6 in clock cycles

#define MAXUTBOARD		8
#define MAX_WAVEFORM	16


#define SETSOFTWARERECTIFICATION	3001
	// wParam: 0 = SW Rectification Off, 1 = On
